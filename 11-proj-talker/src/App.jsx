  ...
  return (
    <main className="font-semibold h-screen bg-white">
      ...
      <div className={`flex flex-col items-center ${open ? 'ml-[20%] w-[80%]' : 'w-full'} h-screen relative`}>
        ...
        <div className="w-full flex-1 pt-10 p-2 mb-5 text-white rounded-3xl overflow-auto pb-12" style={{ scrollbarWidth: 'none' }}>
          {!qlist.length && (<h1 className="text-xl text-black text-center">Welcome!</h1>)}
          {ind.map((i) => (
            <div key={i}>
              <div className="flex justify-end">
                <p className="rounded-xl p-2 bg-[#1689fc] text-white mb-2 text-end max-w-xl break-words whitespace-pre-line">{qlist[i]}</p>
              </div>
        ...
        <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 bg-zinc-200 ${open ? 'w-3/4' : 'w-1/2'} p-1 pr-5 text-black rounded-3xl border border-zinc-700 flex h-16 z-50`}>
          <input type="text" placeholder="Ask me anything" value={question} onChange={(e) => setQuestion(e.target.value)} onKeyUp={(e) => { if (e.key === "Enter" && loading === false) clicked() }} className="w-full h-full p-3 outline-none bg-transparent rounded-3xl" />
          <button
